...
from .mask_utils import modality_ids_to_cross_modality_attention_mask, combine_attention_masks
...
class LlavaQwenWithAlternatingCrossAttnConfig(Qwen2Config):
    model_type = "llava_qwen_with_alternating_cross_attn"


class LlavaQwenWithAlternatingCrossAttnModel(LlavaMetaModel, Qwen2Model):
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                layer_outputs = self._gradient_checkpointing_func(
                    ...
                    hidden_states,
                    attention_mask if condition else cross_modality_attention_mask,
                    position_ids,
                    past_key_values,
                    output_attentions,
                    use_cache,
                ...
                layer_outputs = decoder_layer(
                    hidden_states,
                    attention_mask=attention_mask if condition else cross_modality_attention_mask,
...
class LlavaQwenWithAlternatingCrossAttnForCausalLM(Qwen2ForCausalLM, LlavaMetaForCausalLM):
    config_class = LlavaQwenWithAlternatingCrossAttnConfig

    def __init__(self, config):
        # super(Qwen2ForCausalLM, self).__init__(config)
        Qwen2ForCausalLM.__init__(self, config)
        config.model_type = "llava_qwen_with_alternating_cross_attn"
        config.rope_scaling = None

        self.model = LlavaQwenWithAlternatingCrossAttnModel(config)
    ...
    def generate(
        ...
        **kwargs,
        ...
        if images is not None:
            (inputs, position_ids, attention_mask, _, inputs_embeds, _, modality_ids) = self.prepare_inputs_labels_for_multimodal(inputs, position_ids, attention_mask, None, None, images, modalities, image_sizes=image_sizes)
        else:
    ...
    def prepare_inputs_for_generation(self, input_ids, past_key_values=None, inputs_embeds=None, **kwargs):
        ...
        modality_ids = kwargs.pop("modality_ids", None)
        ...
        if modality_ids is not None:
            inputs["modality_ids"] = modality_ids
        return inputs


AutoConfig.register("llava_qwen_with_alternating_cross_attn", LlavaQwenWithAlternatingCrossAttnConfig)
AutoModelForCausalLM.register(LlavaQwenWithAlternatingCrossAttnConfig, LlavaQwenWithAlternatingCrossAttnForCausalLM)
